def main(p,n,y,X,A,b,c,K,xrelax,max_cputime=600):
    """IBB+ with lb using recycled echelon form for an array of K values

       K: list of k values in the increasing order
       xrelax: xRelaxedOpt
    """
    import sys
    import numpy as np
    ...
    #print('Ab:',Ab)
    num_k=len(K)  # no. of k values to process
    XBEST=np.zeros((p,num_k))  # initialization
    FBEST=np.zeros(num_k)
    Kflag=np.zeros(num_k) # unique for each box, Kflag[i]=1 box got deleted for Kflag[i], =0 not 
    isKiDone=np.zeros(num_k) # vector to keep track with K[i] got processed, if sum(isKiDone)=num_k, we can stop
    stopflag=0 
    B0=np.ones(p,dtype=int) # initial box of integer ones
    num_iter=0
    num_box=1
    for i in range(num_k):
        XBEST[:,i],FBEST[:,i]=getfeasiblept(p,n,y,X,A,b,c,K[i],B0,xrelax,absxrelax)
    ...
    heapq.heappush( L, ( fxlb, box_age_ctr , B0 , 0, p, 0,  xlb, Kflag) )  # add the intial box to the list
    ...
    while True:
        ...
        Y = heapq.heappop(L) # Y[0]=fxlb, Y[1]= age counter,  Y[2]=box, Y[3]=#0, Y[4]=#1, Y[5]=#2, Y[6]=xlb, Y[7]=KFLAG
        #print('Selected node Y:',Y)
        num_box -= 1
        Y_Kflag=Y[7].copy()
        for i in range(num_k):
            if isKiDone[i]==1: # if K[i] has been processed
                Y_Kflag[i]=1

        # branch over Y
        box_age_ctr, num_box, fbest, xbest, L = branch(p,n,y,X,A,b,c,K,L,Y,E0,CE0,Ab,XBEST,FBEST,xhat0,fxhat0,xrelax,absxrelax,Y_Kflag,isKiDone,npiv0,num_box,box_age_ctr,num_k)
    ...
    for j in range(num_k):
        xtemp=np.zeros((p,1))
        xtemp[ipiv0]=xbest[:npiv0]
        xtemp[ipiv1]=xbest[npiv0:p]
        XOUT[:,j]=xtemp
...
def branch(p,n,y,X,A,b,c,K,L,Y,E0,CE0,Ab,XBEST,FBEST,xhat0,fxhat0,xrelax,absxrelax,Y_Kflag,isKiDone,npiv0,num_box,box_age_ctr,num_k):
    """
    ...
    uplimit=np.zeros(num_k-sum(isKiDone))
    ctr=0
    for i in range(num_k):
        if isKiDone[i]==0:
            uplimit[ctr]=p-K[i]-Y[3]
            ctr += 1 
    ...
    for j in range(num_child):
        ...
        #print('tempV0:',tempV0)
        V0=[tempV0 , Y[3]+1, Y[4]-1, Y[5], Y_Kflag ] # initialization 
        #print('V0:',V0)
        tempV2=Y[2].copy()
        tempV2[jhat]=2   # child box with 2 flag
        V2=[tempV2 , Y[3], Y[4]-1, Y[5]+1, Y_Kflag ]
