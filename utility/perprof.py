...
--------
May 16, 2025
Downloaded from https://github.com/sdrelton/perfprof/blob/master/perfprof.py
...
def main(data, linespecs=None, linewidth=1.5, thmax=None,
             thlabel=r'$\tau$', plabel='$CDF$', tol=np.double(1e-8),
             legendnames=None, legendpos=0,
             fontsize=15, tickfontsize=14, legendfontsize=10,
    ...
    for alt in range(n):  # for each alternative
        ...
        k = np.array(np.floor(np.arange(0, r, 0.5)), dtype=int)
        x = theta[k[1:]]
        y = prob[k[0:-1]]
        ...
        if legendnames is None:
            gen_legend = f"Solver {alt}"
            plt.plot(x, y, linewidth=linewidth, label=gen_legend)
        else:
